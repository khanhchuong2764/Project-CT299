                ...
                <form action="/admin/product/edit/{{$record->id}}" method="Post" enctype="multipart/form-data">
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input type="text" name="id_product" class="form-control" value='{{$record->id_product}}' placeholder="Nhập Mã Sản Phẩm">
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <select name="category_id" class="form-control" id="parentID">
                                <option  value="0">--Chọn Danh Mục--</option>
                                {!! App\Helpers\CategoryHelper::MenuEdit($category,$record->category_id) !!}
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select name="id_producers" class="form-control">
                                ...
                                @foreach ($producers as $item)
                                    <option value={{$item->id}} {{$item->id == $record->id_producers ? "selected" :""}} >{{$item->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select name="id_unit" class="form-control">
                                ...
                                @foreach ($units as $item)
                                    <option value={{$item->id}} {{$item->id == $record->id_unit ? "selected" :""}}>{{$item->title}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="title">Quy Cách</label>
                            <input type="text" name="specifications" class="form-control" value='{{$record->specifications}}' placeholder="Nhập Quy Cách">
                        </div>
                        {{-- <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="partials" class="form-control tinymece" rows ="5">{{$record->partials}}</textarea>
                        </div>
                        <div class="form-group">
                            <label for="content">Công Dụng</label>
                            <textarea name="uses" class="form-control tinymece" rows ="5">{{$record->uses}}</textarea>
                        </div>
                        <div class="form-group">
                            <label for="content">Cách Dùng</label>
                            <textarea name="howuse" class="form-control tinymece" rows ="5">{{$record->howuse}}</textarea>
                        </div>
                        <div class="form-group">
                            <label for="content">Mô Tả</label>
                            <textarea name="description"  class="form-control tinymece" rows ="5">{{$record->description}}</textarea>
                        </div>
                        <div class="form-group">
                            <label for="content">Lưu Ý</label>
                            <textarea name="note" class="form-control tinymece" rows ="5">{{$record->note}}</textarea>
                        </div>
                        <div class="form-group">
                            <label for="content">Bảo Quản</label>
                            <textarea name="preserve" class="form-control tinymece" rows ="5">{{$record->preserve}}</textarea>
                        </div>
